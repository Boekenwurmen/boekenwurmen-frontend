...
export default class Stories {
    ...
    static _getPageStoryLocal(page){
        switch (page) {
            ...
            case 5: return "I don't know how to open this library door — I need a code. Can you tell me the code?"
    ...
    static _getPageOptionsLocal(page){
        switch (page) {
            // When choosing from page 1 we first show the short follow-up (page 2)
            // and only then proceed to the name prompt (page 3). Both choices
            // lead to the intermediate page so the player sees the extra text.
            case 1: return [
                {toPage:2, name:"You hit the ground out of frustration"},
                {toPage:2, name:"You scream in pain from the sand in your eyes"},
            ]
            case 2: return [
                // After the short follow-up on page 2, both choices proceed to
                // the hatch/story progression which leads to the name prompt (page 3).
