<script>
	...
	const errorOptionUnknown = $derived(
		errorOptions.find((opt) => opt.name === 'service unavailable')
	);
	let codeValue = $state('');
	let message = $state('');
	...
	$effect(() => {
		...
		optionsPromise
			...
			.catch((err) => {
				...
				options = [{ toPage: 0, type: 'page', name: 'Go back' }];
			});
	});
	...
	async function submitCode() {
		...
		try {
			...
			console.log('[ChooseActionWindow] submitCode start', {
				hasCode: !!codeValue,
			...
			if (!clientContext.id) {
				...
				if (status === 201 && data && data.success && data.client) {
					...
					console.warn('[ChooseActionWindow] register client response', {
				...
				if (status >= 200 && status < 300 && data && data.success && data.client) {
					...
					console.warn('[ChooseActionWindow] save code response', {
						success: !!data?.success,
						error: data?.message
					});
					if (followOption(pageContext, errorOptionUnknown)) return;
...
<div class="story-box">
	<label for="code-input" class="label-dark mb-2 block"
		>Enter your personal library code (password)</label
	>
	...
	{#if message}
		<p class="message-text mt-2">{message}</p>
	{/if}
</div>
